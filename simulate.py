...
from _rotatedplanarxzycode import RotatedPlanarXZYCode
from _rotatedplanarxzyrmpsdecoder import RotatedPlanarXZYRMPSDecoder
import logging
import numpy as np
...
max_runs = 1000
filename = 'eta_100_d9-51.json'
#run simulation
codes = [RotatedPlanarXZYCode(distance) for distance in range(min_code_distance, max_code_distance+1, interval)]
error_probabilities = np.linspace(min_error_prob, max_error_prob, num_points)
decoder = RotatedPlanarXZYRMPSDecoder(chi=12)
