...
def save_data(filename, data):
    ...
    with open(os.path.join(save_path, filename),"w", encoding='utf-8') as file:
            for dict in data:
                file.write(json.dumps(dict, cls=NpEncoder))
                file.write("\n")
...
def _run_once(mode, code, time_steps, error_model, decoder, error_probability, measurement_error_probability, rng):
    ...
    if decoding.recovery is not None:
        ...
        logical_xs = code.logical_xs
        logical_ys = code.logical_xs^code.logical_zs
        logical_zs = code.logical_zs
        logicals = np.vstack((logical_xs, logical_ys, logical_zs))

        resolved_logical_commutations = pt.bsp(recovered, logicals.T)
