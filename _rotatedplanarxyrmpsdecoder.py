...
from _rotatedplanarxycode import RotatedPlanarXYCode

logger = logging.getLogger(__name__)


@cli_description('Rotated MPS ([chi] INT >=0, [mode] CHAR, ...)')
class RotatedPlanarXYRMPSDecoder(Decoder):
    r"""
    ...
    * Contracting by columns (i.e. truncating vertical links) may give different coset probabilities to contracting by
    ...
    def sample_recovery(cls, code, syndrome):
        ...
        for plaq_index in plaquette_indices:
            ...
            if code.is_y_plaquette(plaq_index):
                ...
                sample_recovery.site('Y', *((site_x, site_y) for site_y in site_y_range))
        # return sample
    ...
    def label(self):
        ...
        return 'Rotated planar XY RMPS ({})'.format(', '.join('{}={}'.format(k, v) for k, v in params if v))
    ...
    class TNC:
        ...
        def create_q_node(self, prob_dist, f, h_node, even_column, compass_direction=None):
            ...
            * H-nodes have Y-plaquettes above and below (i.e. in NE and SW directions).
            * V-nodes have Y-plaquettes on either side (i.e. in NW and SE directions).
